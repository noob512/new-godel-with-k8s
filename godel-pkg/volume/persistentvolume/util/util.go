...
func FindMatchingVolume(
	...
	requestedClass := helper.GetPersistentVolumeClaimClass(claim)
	...
	for _, volume := range volumes {
		...
		err := volumeutil.CheckNodeAffinity(volume, nodeLabels)
		if err != nil {
		...
		if helper.GetPersistentVolumeClass(volume) != requestedClass {
			continue
		}
		if !nodeAffinityValid {
			continue
