...
// StartRecordingToSink 将指定事件广播器接收到的事件发送到给定的接收端 (sink)。
// 该方法会启动两个记录流程（如果相应的广播器和客户端不为 nil）：
// 1. 使用 e.broadcaster 和 e.eventClient 启动记录到 sink。
// 2. 使用 e.coreBroadcaster 和 e.coreEventClient 启动记录到核心事件 sink。
// stopCh 通道用于控制何时停止记录。
func (e *eventBroadcasterAdapterImpl) StartRecordingToSink(stopCh <-chan struct{}) {
	// 检查并启动第一个事件记录流程（如果 broadcaster 和 eventClient 存在）
	if e.broadcaster != nil && e.eventClient != nil {
		e.broadcaster.StartRecordingToSink(stopCh)
	}
	// 检查并启动第二个事件记录流程（如果 coreBroadcaster 和 coreEventClient 存在）
	if e.coreBroadcaster != nil && e.coreEventClient != nil {
		// 创建一个核心事件 sink 实例，并将其传递给 coreBroadcaster 的 StartRecordingToSink 方法
		// 注意：这里直接传入了 &v1core.EventSinkImpl{Interface: e.coreEventClient.Events("")}，这与方法签名可能不完全一致
		// 通常 StartRecordingToSink 接受的是一个 stop channel，而不是 sink 本身。
		// 此处代码逻辑可能存在错误或与标准实现不同。
