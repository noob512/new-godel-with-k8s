...
func (m *FakeNodeHandler) Patch(_ context.Context, name string, pt types.PatchType, data []byte, _ metav1.PatchOptions, subresources ...string) (*v1.Node, error) {
	...
	if err != nil {
		klog.InfoS("Failed to marshal the node", "node", klog.KObj(&nodeCopy), "err", err)
		return nil, nil
	}
	var originalNode v1.Node
	if err = json.Unmarshal(originalObjJS, &originalNode); err != nil {
		klog.InfoS("Failed to unmarshal the original object", "err", err)
		return nil, nil
	}
	...
	case types.JSONPatchType:
		...
		if err != nil {
			klog.InfoS("Error occurred during DecodePatch", "err", err)
			return nil, nil
		}
		if patchedObjJS, err = patchObj.Apply(originalObjJS); err != nil {
			klog.InfoS("Error occurred during patch apply", "err", err)
			return nil, nil
		}
	case types.MergePatchType:
		if patchedObjJS, err = jsonpatch.MergePatch(originalObjJS, data); err != nil {
			klog.InfoS("Error occurred during MergePatch", "err", err)
			return nil, nil
		}
	case types.StrategicMergePatchType:
		if patchedObjJS, err = strategicpatch.StrategicMergePatch(originalObjJS, data, originalNode); err != nil {
			klog.InfoS("Error occurred during StrategicMergePatch", "err", err)
			return nil, nil
		}
	default:
		klog.InfoS("Unknown Content-Type header", "patch", pt)
		return nil, nil
	}

	var updatedNode v1.Node
	if err = json.Unmarshal(patchedObjJS, &updatedNode); err != nil {
		klog.InfoS("Failed to unmarshal the patched object", "err", err)
		return nil, nil
	}
...
func (f *FakeRecorder) AnnotatedEventf(obj runtime.Object, annotations map[string]string, eventtype, reason, messageFmt string, args ...interface{}) {
	f.Eventf(obj, nil, eventtype, reason, "", messageFmt, args...)
}

func (f *FakeRecorder) generateEvent(obj runtime.Object, timestamp metav1.Time, eventtype, reason, message string) {
	f.Lock()
	defer f.Unlock()
	ref, err := ref.GetReference(apiserver.Scheme, obj)
	if err != nil {
		klog.InfoS("Error occurred while getting reference", "err", err)
		return
	}
