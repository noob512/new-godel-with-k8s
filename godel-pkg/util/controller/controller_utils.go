...
func (r *ControllerExpectations) DeleteExpectations(controllerKey string) {
	if exp, exists, err := r.GetByKey(controllerKey); err == nil && exists {
		if err := r.Delete(exp); err != nil {
			klog.InfoS("Error occurred when deleting expectations for controller", "controllerKey", controllerKey, "err", err)
		}
	}
}
...
func (r *ControllerExpectations) SatisfiedExpectations(controllerKey string) bool {
	if exp, exists, err := r.GetExpectations(controllerKey); exists {
		if exp.Fulfilled() {
			klog.V(4).InfoS("Controller expectations fulfilled", "expectation", exp)
			return true
		} else if exp.isExpired() {
			klog.V(4).InfoS("Controller expectations expired", "expectation", exp)
			return true
		} else {
			klog.V(4).InfoS("Controller was still waiting on expectations", "expectation", exp)
			return false
		}
	} else if err != nil {
		klog.InfoS("Error occurred while checking expectations, forcing sync", "err", err)
	} else {
		...
		klog.V(4).InfoS("Controller either never recorded expectations, or the ttl expired", "controllerKey", controllerKey)
	}
...
func (r *ControllerExpectations) SetExpectations(controllerKey string, add, del int) error {
	...
	klog.V(4).InfoS("Set expectations", "expectation", exp)
	return r.Add(exp)
}
...
func (r *ControllerExpectations) LowerExpectations(controllerKey string, add, del int) {
	if exp, exists, err := r.GetExpectations(controllerKey); err == nil && exists {
		...
		klog.V(4).InfoS("Lowered expectations", "expectation", exp)
	}
}
...
func (r *ControllerExpectations) RaiseExpectations(controllerKey string, add, del int) {
	if exp, exists, err := r.GetExpectations(controllerKey); err == nil && exists {
		exp.Add(int64(add), int64(del))
		// The expectations might've been modified since the update on the previous line.
		klog.V(4).InfoS("Raised expectations", "expectation", exp)
	}
}
...
func (u *UIDTrackingControllerExpectations) ExpectDeletions(rcKey string, deletedKeys []string) error {
	...
	klog.V(4).InfoS("Controller was waiting on deletions", "controllerKey", rcKey, "deletedKeys", deletedKeys)
	u.uidStoreLock.Lock()
	defer u.uidStoreLock.Unlock()

	if existing := u.GetUIDs(rcKey); existing != nil && existing.Len() != 0 {
		klog.InfoS("Clobbering existing delete keys", "existingKeys", existing)
	}
...
func (u *UIDTrackingControllerExpectations) DeletionObserved(rcKey, deleteKey string) {
	...
	if uids != nil && uids.Has(deleteKey) {
		klog.V(4).InfoS("Controller received delete for pod", "controllerKey", rcKey, "deleteKey", deleteKey)
...
func (u *UIDTrackingControllerExpectations) DeleteExpectations(rcKey string) {
	...
	if uidExp, exists, err := u.uidStore.GetByKey(rcKey); err == nil && exists {
		if err := u.uidStore.Delete(uidExp); err != nil {
			klog.InfoS("Failed to delete the UID expectations for controller", "controllerKey", rcKey, "err", err)
		}
	}
}
...
func (r RealPodControl) createPods(nodeName, namespace string, template *v1.PodTemplateSpec, object runtime.Object, controllerRef *metav1.OwnerReference) error {
	...
	if err != nil {
		klog.InfoS("The parentObject did not have ObjectMeta", "err", err)
		return nil
	}
	klog.V(3).InfoS("The pod was created", "accessor", accessor.GetName(), "pod", klog.KObj(newPod))
...
func (r RealPodControl) DeletePod(namespace string, podID string, object runtime.Object) error {
	...
	klog.V(3).InfoS("Started to delete the pod", "accessor", accessor.GetName(), "pod", klog.KRef(namespace, podID))
	if err := r.KubeClient.CoreV1().Pods(namespace).Delete(context.TODO(), podID, metav1.DeleteOptions{}); err != nil {
		if apierrors.IsNotFound(err) {
			klog.V(4).InfoS("The pod had already been deleted", "pod", klog.KRef(namespace, podID))
			return err
		}
...
func FilterActivePods(pods []*v1.Pod) []*v1.Pod {
	...
	for _, p := range pods {
		if IsPodActive(p) {
			...
			klog.V(4).InfoS("Ignored inactive pod",
				"pod", klog.KObj(p), "podPhase", p.Status.Phase, "podDeletionTimestamp", p.DeletionTimestamp)
		}
	}
	return result
}
