...
// responsibleForPod checks if the given pod should be handled by the specified scheduler.
...
// FilteringUpdate 是一个通用的工具函数，用于根据过滤条件和对象的新旧状态变化，
...
func FilteringUpdate(
	...
	// 1. 检查旧的 Pod 对象是否满足过滤条件。
	older := filterFn(oldPod)
	// 2. 检查新的 Pod 对象是否满足过滤条件。
	newer := filterFn(newPod)

	// 3. 根据新旧 Pod 是否满足过滤条件，执行相应的操作。
	switch {
	// 情况 1: 新旧 Pod 都满足过滤条件。
	// 这通常意味着一个已知的、受关注的对象发生了状态变化。
	// 动作: 调用 updateFunc 函数来处理更新。
	case newer && older:
		updateFunc(oldPod, newPod)

	// 情况 2: 新 Pod 满足条件，但旧 Pod 不满足。
	// 这通常意味着一个之前不受关注的对象，现在变成了需要关注的对象（例如，标签被修改），
	// 或者是一个全新的、需要关注的对象出现了。
	// 动作: 调用 addFunc 函数将其添加。
	case newer && !older:
		addFunc(newPod)

	// 情况 3: 旧 Pod 满足条件，但新 Pod 不满足。
	// 这通常意味着一个之前受关注的对象，现在不再需要被关注（例如，标签被修改或对象被删除）。
	// 动作: 调用 deleteFunc 函数将其删除。
	case !newer && older:
		deleteFunc(oldPod)

	// 情况 4: 新旧 Pod 都不满足过滤条件。
	// 这意味着这个对象的变化与当前处理逻辑无关。
	// 动作: 什么都不做 (do nothing)。
	default:
		// do nothing
	}
