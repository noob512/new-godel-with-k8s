...
// Run 启动优先级队列（PriorityQueue）的后台处理协程。
// 该函数启动多个定时任务协程，负责清理和刷新队列中的不同状态的 Pod/Unit，
// 确保调度器能够及时处理过期的、可重新调度的或长时间等待的 Pod。
// 这是 PriorityQueue 维护其内部状态和保证调度效率的关键方法。
func (p *PriorityQueue) Run() {
	klog.Info("启动PriorityQueue")
	// 启动后台协程，定期刷新退避队列（Backoff Queue）中已完成退避的 Pod/Unit
	// 每秒检查一次，将已完成退避等待的 Pod/Unit 重新放回就绪队列进行调度
	go wait.Until(p.flushBackoffQCompleted, 1.0*time.Second, p.stop)
	
	// 启动后台协程，定期刷新不可调度队列（Unschedulable Queue）中的遗留 Pod/Unit
	// 每 30 秒检查一次，将长时间停留在不可调度队列中的 Pod/Unit 移动到就绪队列重新尝试调度
	// 这有助于处理因集群状态变化而可能变得可调度的 Pod
	go wait.Until(p.flushUnschedulableQLeftover, 30*time.Second, p.stop)
	
	// TODO: 讨论是否移除 flushScheduledUnitsInWaitingQ 和 flushWaitingPodsList 功能
	// 启动后台协程，定期刷新等待队列（Waiting Queue）中已调度的 Unit
	// 每 2 秒检查一次，处理等待队列中状态已改变的调度单元
	go wait.Until(p.flushScheduledUnitsInWaitingQ, 2.0*time.Second, p.stop)
	
	// 启动后台协程，定期刷新等待 Pod 列表（Waiting Pods List）
	// 每 20 秒检查一次，处理长时间等待的 Pod
...
// flushBackoffQCompleted 刷新退避队列（Backoff Queue）中已完成退避等待的调度单元。
// 该函数会遍历退避队列的头部，检查队列中的调度单元是否已经完成退避时间，
// 如果完成退避，则将该单元从退避队列移除并添加到就绪队列（Ready Queue）中，
// 以便调度器可以重新尝试调度这些单元。
// 这是 PriorityQueue 确保退避单元能够及时重新进入调度流程的关键方法。
func (p *PriorityQueue) flushBackoffQCompleted() {
	// 获取队列锁，确保并发安全
	p.lock.Lock()
	defer p.lock.Unlock()
	
	// 循环处理退避队列中的单元，直到队列为空或遇到仍在退避中的单元
	for {
		// 查看退避队列的头部单元，但不从队列中移除
		...
		if p.boHandler.isUnitBackoff(unitInfo) {
			// 如果仍在退避中，停止处理，因为队列是按时间排序的，后续单元也仍在退避中
			return
		}
		...
		// 将已完成退避的单元添加到就绪队列中，准备重新调度
		if err := p.readyQ.Add(unitInfo); err != nil {
			// 如果添加到就绪队列失败，记录错误日志并返回
...
// flushWaitingPodsList 刷新等待 Pod 列表（Waiting Pods List）中的 Pod，
// 将它们转换为调度单元并根据调度准备状态移动到相应的队列中。
// 该函数会遍历等待列表中的所有 Pod，尝试将它们转换为调度单元，
// 然后根据单元是否准备好被调度，将其放入就绪队列或继续留在等待队列。
// 这是 PriorityQueue 确保等待的 Pod 能够及时响应集群状态变化的关键方法。
func (p *PriorityQueue) flushWaitingPodsList() {
	// 获取队列锁，确保并发安全
	klog.Info("启动flushWaitingPodsList-1")
	p.lock.Lock()
	defer p.lock.Unlock()
	...
	for _, unit := range unitsToMove {
		...
		klog.InfoS("Flushed scheduling units in the PriorityQueue", "subCluster", p.subCluster, "qos", p.qos, "unitKey", unit.UnitKey, "oldQueue", p.waitingPodsList, "newQueue", queue)
