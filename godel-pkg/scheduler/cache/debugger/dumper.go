...
func (d *CacheDumper) dumpNodes() {
	dump := d.cache.Dump()
	klog.V(4).InfoS("Dumped the node info in scheduler cache", "numNodes", len(dump.Nodes))
	for name, nodeInfo := range dump.Nodes {
		klog.V(4).InfoS(d.printNodeInfo(name, nodeInfo))
	}
}
...
func (d *CacheDumper) dumpSchedulingQueue() {
	...
	pendingPods := d.unitPodQueue.PendingPods()
	klog.V(4).InfoS("Dumped the pending pod info in scheduler cache", "numPendingPods", len(pendingPods))
	for _, p := range pendingPods {
		podData.WriteString(printPod(p))
	}
	klog.V(4).InfoS("Dumped the pending pod detail in scheduler cache", "pendingPodsDetail", podData.String())
}
...
func (d *CacheDumper) printNodeInfo(name string, n api.NodeInfo) string {
	...
	//  Deleted: false
	//  Gauranteed Requested Resources:xxx
	//  Gauranteed Allocatable Resources: xxx
	...
	// Dump node's numa topology infomation
