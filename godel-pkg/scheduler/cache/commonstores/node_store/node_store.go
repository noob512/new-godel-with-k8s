...
import (
	...
	//utilfeatures "k8s.io/kubernetes/godel-pkg/util/features"
...
// AddPod 将一个 Pod 添加到 NodeStore 中对应的节点信息中。
// 该函数主要处理已绑定（Bound）或假设（Assumed）状态的 Pod，
// 并将其关联到目标节点，更新节点的资源使用情况和 Pod 列表。
// 这是调度器缓存中节点资源管理的重要组成部分。
func (s *NodeStore) AddPod(pod *v1.Pod) error {
	// 只处理已绑定到节点的 Pod 或由 Godel 调度器假设的 Pod
	// 其他状态的 Pod（如待调度、已删除等）直接返回，不进行处理
	if !podutil.BoundPod(pod) && !podutil.AssumedPodOfGodel(pod, s.handler.SchedulerType()) {
		klog.Info("直接返回")
		return nil
	}
	
	// 从 Pod 的 NodeName 字段获取目标节点名称
	nodeName := utils.GetNodeNameFromPod(pod)
	// 如果节点名称为空，说明 Pod 没有被正确分配到节点
	if nodeName == "" {
		// 记录警告日志，指示 Pod 被分配到了空节点（这种情况通常是异常的）
		klog.InfoS("WARN: Pod was assigned to empty node", "pod", pod.Name)
		return nil
	}
	
	// 获取或创建对应节点的 NodeInfo 对象
	// 如果节点不存在，则创建一个新的 NodeInfo 实例
	nodeInfo := s.getOrCreateNode(nodeName)
	// 将 Pod 添加到节点信息中，更新节点的资源统计和 Pod 列表
	nodeInfo.AddPod(pod)
	return nil
}
...
// UpdateSnapshot 更新快照存储中的节点信息
// 该方法将缓存中的节点信息同步到快照存储中，确保快照与缓存保持一致
func (cacheStore *NodeStore) UpdateSnapshot(store commonstore.Store) error {
	// 将传入的通用存储转换为 NodeStore 类型
	snapshotStore := store.(*NodeStore)
	// 通过框架工具函数获取缓存和快照的底层存储对象
	...
	// 检查是否启用了调度器子集群并发调度功能
	subClusterConcurrentSchedulingEnabled := false
