...
func RunE2ETests(t *testing.T) {
	...
	if framework.TestContext.ReportDir != "" {
		...
		if err := os.MkdirAll(framework.TestContext.ReportDir, 0o755); err != nil {
			klog.ErrorS(err, "Failed to create the report directory")
		} else {
	...
	klog.InfoS("Started e2e run on Ginkgo node", "runID", framework.RunID, "ginkgoNode", config.GinkgoConfig.ParallelNode)
...
func setupSuite() {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to load client")
	...
	if framework.TestContext.CleanStart {
		...
		klog.InfoS("Waited for deletion of the namespaces", "namespaces", deleted)
