...
func NewMetricsGrabber(c clientset.Interface, ec clientset.Interface, kubelets bool, scheduler bool, controllers bool, apiServer bool, clusterAutoscaler bool) (*Grabber, error) {
	...
	if len(podList.Items) < 1 {
		klog.InfoS("WARN: Failed to find any pods in the namespace to grab metrics from", "namespace", metav1.NamespaceSystem)
	}
	...
	if kubeScheduler == "" {
		scheduler = false
		klog.InfoS("WARN: Failed to find kube-scheduler pod. Grabbing metrics from kube-scheduler is disabled")
	}
	if kubeControllerManager == "" {
		controllers = false
		klog.InfoS("WARN: Failed to find kube-controller-manager pod. Grabbing metrics from kube-controller-manager is disabled")
	}
	if ec == nil {
		klog.InfoS("WARN: Did not receive an external client interface. Grabbing metrics from ClusterAutoscaler is disabled")
	}
