...
func WaitTimeoutForPodUnschedulableInNamespace(c clientset.Interface, podName, namespace string, timeout time.Duration) error {
	return wait.PollImmediate(poll, timeout, func() (done bool, err error) {
		running, err := podRunning(c, podName, namespace)()
		if err != nil {
			return false, err
		}
