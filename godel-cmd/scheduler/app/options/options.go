...
func (o *Options) ApplyTo(c *schedulerappconfig.Config) error {
	if len(o.ConfigFile) == 0 {
		klog.Info("选择路径1-len(o.ConfigFile) == 0")
...
// Config 根据当前 Options 配置生成调度器应用所需的完整运行时配置（*schedulerappconfig.Config）。
// 该函数负责初始化安全服务（如 TLS 证书）、创建 Kubernetes 客户端、设置事件广播器、
// 配置 Leader 选举（如启用）、并构建各类 InformerFactory。
func (o *Options) Config() (*schedulerappconfig.Config, error) {
	// 如果启用了安全服务（SecureServing），尝试为 localhost 和 127.0.0.1 自动生成自签名证书（若未提供）。
	// 这通常用于 HTTPS 监听端口（如健康检查、指标、/configz 等）。
	...
	// 将命令行选项（o）应用到配置结构体 c 中。
	if err := o.ApplyTo(c); err != nil {
		return nil, err
	}

	// 创建多个 Kubernetes 客户端：
	// - client：通用客户端，用于核心资源（Pod、Node 等）。
	// - leaderElectionClient：专用于 Leader 选举的客户端（通常权限更小，更安全）。
	// - eventClient：用于发送事件（Events）的客户端。
	// - godelCrdClient：用于操作 Godel 调度器自定义资源（CRD）的客户端。
	// - katalystCrdClient：用于操作 Katalyst（阿里巴巴资源调度增强）相关 CRD 的客户端。
	client, _, eventClient, godelCrdClient, katalystCrdClient, err := createClients(
		c.ComponentConfig.ClientConnection,
		o.Master,
	...
	// 初始化事件广播器，用于将调度器产生的事件发送到 API Server。
	...
	c.Client = client
	// 创建通用资源的 SharedInformerFactory（缓存资源，减少 API Server 压力）。
	...
	// Katalyst CRD 相关客户端和 InformerFactory。
