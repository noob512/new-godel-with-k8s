...
// AssumedPodOfGodel 检查一个 Pod 是否是由指定的 Godel 调度器假设（Assume）的 Pod。
// 该函数通过三个条件来判断：
// 1. Pod 是否符合 Godel 调度器管理的资源类型和启动器要求
// 2. Pod 是否由指定的调度器负责（通过调度器名称匹配）
// 3. Pod 是否处于假设状态（即已被调度器假设但尚未绑定到节点）
// 这是 Godel 调度器进行缓存管理和调度决策时的重要判断逻辑。
func AssumedPodOfGodel(pod *v1.Pod, schedulerName string) bool {
	// 综合判断三个条件：
	// 1. LegalPodResourceTypeAndLauncher(pod): 检查 Pod 的资源类型和启动器是否符合 Godel 调度器管理范围
	// 2. responsibleForPod(pod, schedulerName): 检查 Pod 是否由当前调度器负责调度
	// 3. AssumedPod(pod): 检查 Pod 是否处于假设状态（即已调度但未绑定）
	if LegalPodResourceTypeAndLauncher(pod) &&
		responsibleForPod(pod, schedulerName) &&
		AssumedPod(pod) {
		return true
...
// AssumedPod 检查给定的 Pod 是否处于假设（Assumed）状态。
// 在 Kubernetes 调度器中，假设状态是指 Pod 已被调度器选中节点并假设绑定，
// 但尚未实际向 API Server 发起绑定请求的状态。
// 该函数通过检查 Pod 的注解、调度器信息、节点分配状态等来判断 Pod 是否为假设状态。
// 这是调度器缓存管理和调度决策中的重要判断逻辑。
func AssumedPod(pod *v1.Pod) bool {
	// 综合判断四个条件来确定 Pod 是否为假设状态：
	// 1. pod.Annotations != nil: 确保 Pod 有注解
	// 2. pod.Annotations[PodStateAnnotationKey] == string(PodAssumed): 
	//    检查 Pod 状态注解是否标记为 "Assumed" 状态
	// 3. len(pod.Annotations[SchedulerAnnotationKey]) != 0: 
	//    检查 Pod 是否被分配了调度器（有调度器名称）
	// 4. assumedOrNominatedNodeIsSet(pod): 
	//    检查 Pod 是否设置了假设的节点或被提名的节点
	// 5. len(pod.Spec.NodeName) == 0: 
	//    检查 Pod 的 Spec.NodeName 为空，即尚未实际绑定到节点
	if pod.Annotations != nil &&
		pod.Annotations[PodStateAnnotationKey] == string(PodAssumed) &&
		len(pod.Annotations[SchedulerAnnotationKey]) != 0 &&
		assumedOrNominatedNodeIsSet(pod) &&
		len(pod.Spec.NodeName) == 0 {
		klog.Info("即将return true在AssumedPod")
		return true
	}
	klog.Info("即将return false在AssumedPod")
	return false
}
